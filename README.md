...
[![Validation: 177 checks](https://img.shields.io/badge/documented%20figures-177%20checked-1baf7a.svg)](validate.py)
...
├── validate.py                   re-derives all 177 documented figures from the data
...
```bash
sha256sum -c CHECKSUMS.sha256   # file integrity — 36 files
python3 validate.py             # every documented figure, re-derived
```

`validate.py` uses only the Python standard library. It re-derives all **177**
